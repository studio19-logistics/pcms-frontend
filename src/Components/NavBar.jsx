...
function formatAction(action, entityName) {
  const labels = {
    ...
    project_deleted: 'Project deleted', project_reassigned: 'Project reassigned',
    invoice_created: 'Invoice added', invoice_updated: 'Invoice updated',
    invoice_deleted: 'Invoice deleted', milestones_updated: 'Milestones updated',
    milestone_marked_paid: 'Milestone marked paid', milestone_marked_pending: 'Milestone marked pending',
    milestone_snoozed: 'Milestone snoozed', milestone_unsnoozed: 'Milestone unsnoozed',
    client_created: 'Client added', client_updated: 'Client updated',
    client_deleted: 'Client deleted', contact_added: 'Contact added',
    contact_deleted: 'Contact deleted', project_note_added: 'Note added',
    invoice_note_added: 'Note added', project_note_deleted: 'Note deleted',
    ...
    <>
      <header className="bg-surface bg-texture border-b border-surface-border sticky top-0 z-20">
        <div className="px-6 pt-5 pb-4 flex items-center justify-between">
          <div>
            <h1 className="font-serif text-xl text-ink">PCMS</h1>
            <p className="text-xs text-ink-dim">Studio19 Offices</p>
          </div>
          <button
            onClick={openSidebar}
          ...
          {TABS.map(tab => (
            <button
              ...
              className={`text-sm font-medium px-4 py-2 rounded-pill whitespace-nowrap transition ${
                active === tab.id ? 'bg-ink text-surface' : 'text-ink-dim hover:text-ink bg-surface-raised/50'
              }`}
            >
              {tab.label}
            </button>
      ...
      <div className={`fixed top-0 right-0 h-full w-80 bg-surface-card border-l border-surface-border z-50 flex flex-col transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading && activities.length === 0 && <p className="text-xs text-ink-faint px-4 py-6 text-center">Loading...</p>}
          {!loading && activities.length === 0 && <p className="text-xs text-ink-faint px-4 py-6 text-center">No activity yet.</p>}
          {activities.map(a => (
            <div key={a.id} className="px-4 py-3 border-b border-surface-border hover:bg-surface-raised/50 transition">
              ...
              <p className="text-[11px] text-ink-faint mt-0.5">{a.performed_by_name || 'System'} · {timeAgo(a.created_at)}</p>
            </div>
          ))}
        </div>
      </div>
