...
export default function Projects({ onOpenProject }) {
  ...
  async function loadAll() {
    setLoading(true)
    const [projectData, clientData] = await Promise.all([api.getProjects(), api.getClients()])
...
function ProjectRow({ project, currentUserId, isAdmin, onClick, onUpdated, onDeleted }) {
  const [showEdit, setShowEdit] = useState(false)
  const canEdit = isAdmin || project.owner_id === currentUserId
...
function ProjectModal({ project, clients = [], onClose, onCreated, onUpdated }) {
  ...
  async function handleSubmit() {
    ...
    setError('')
    try {
      const payload = { ...form, project_value: Number(form.project_value) }
      const result = isEdit
